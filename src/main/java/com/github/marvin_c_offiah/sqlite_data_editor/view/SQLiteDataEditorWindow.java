...
public class SQLiteDataEditorWindow implements Observer {
	...
	public static final Color DEFAULT_TABLE_ROW_SELECTION_COLOR = new Color(
			0.0f, 0.0f, 1.0f, 0.5f);

	public static final Color TABLE_ROW_INSERT_COLOR = new Color(0.0f, 0.6f,
			0.0f, 0.5f);

	public static final Color TABLE_ROW_UPDATE_COLOR = new Color(1.0f, 1.0f,
			0.0f, 0.5f);

	public static final Color TABLE_ROW_ERROR_COLOR = new Color(1.0f, 0.0f,
			0.0f, 0.5f);
		...
		public SQLiteDatabaseFileChooser(boolean isOpen) {
			...
			addChoosableFileFilter(new FileFilter() {
				...
				public boolean accept(File f) {
					...
					if (extension != null) {
						if (extension.equals("db") || extension.equals("sdb")
								|| extension.equals("sqlite")
								|| extension.equals("db3")
								|| extension.equals("s3db")
								|| extension.equals("sqlite3")
								|| extension.equals("sl3")
								|| extension.equals("db2")
								|| extension.equals("s2db")
								|| extension.equals("sqlite2")
								|| extension.equals("sl2")) {
							return true;
						} else {
							return false;
						}
	...
	protected class SQLiteTable extends JTable {
		...
		protected int getkeyClass(int key) {
			switch (key) {
				case NO_KEY:
					return TABLE_DO_NOTHING_KEY_CLASS;
				case VK_ENTER:
		...
		protected boolean tryRestoreOriginalRow(
				Exception[] exceptionReference) {
			try {
				int rowId = (int) getModel().getValueAt(currentSelection[0],
						getModel().getColumnCount() - 1);
				TreeMap<String, Object> pk = new TreeMap<String, Object>();
				pk.put("_rowid_", rowId);
				TreeMap<String, Object> row = controller
						.getOriginalRow(this.getName(), pk);
				for (String key : row.keySet()) {
					setValueAt(row.get(key), currentSelection[0],
							this.getColumnModel().getColumnIndex(key));
				}
			} catch (Exception e) {
		...
		protected boolean trySaveUpdate(Exception[] exceptionReference) {
			try {
				...
				List<String> primKeyElems = Arrays
						.asList(controller.getPrimaryKey(getName()));
		...
		protected boolean trySaveInsert(Exception[] exceptionReference) {
			try {
				...
				for (int i = 0; i < getColumnCount(); i++) {
					vals.put(getColumnName(i),
							getValueAt(currentSelection[0], i));
				}
	...
	public SQLiteDataEditorWindow(SQLiteDataEditorController controller,
			SQLiteInMemoryDatabase model) throws Exception {
		if (controller == null)
			throw new Exception(
					"The controller must be provided to the data editor.");
	...
	private void initialize() {
		...
		frmSqliteDataEditor.getContentPane().add(tpnTables,
				BorderLayout.CENTER);

		pnlButtons = new JPanel();
		frmSqliteDataEditor.getContentPane().add(pnlButtons,
		...
		btnAddRow.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				int selectedIdx = tpnTables.getSelectedIndex();
				if (selectedIdx > -1) {
					requestStateChange(INSERT_TABLE_ROW_COMMAND,
	...
	public void update(Observable database, Object object) {
		...
		try {
			...
			for (int i = 0; i < tblNames.length; i++) {
				...
				String[] colNames = ksColNames
						.toArray(new String[ksColNames.size()]);
				TreeMap<String, TreeMap<String, String>> importedKeys = ((SQLiteInMemoryDatabase) database)
						.getImportedKeys(tblName);
				String[] refTblNames = importedKeys.keySet()
						.toArray(new String[importedKeys.size()]);
				...
				tcm.removeColumn(tcm.getColumn(
						tbl.getColumnModel().getColumnIndex("_rowid_")));
				...
				for (String refTblName : refTblNames) {
					ArrayList<String> pks = new ArrayList<String>(
							importedKeys.get(refTblName).keySet());
					for (int j = 0; j < tbl.getColumnCount(); j++) {
						...
						if (pks.contains(colName)) {
							ArrayList<TreeMap<String, Object>> colResult = ((SQLiteInMemoryDatabase) database)
									.selectColumns(refTblName, null,
											new String[] { colName }, true);
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(frmSqliteDataEditor, e.getMessage(),
	...
	protected void requestStateChange(int command, SQLiteTable table) {
		...
		boolean rowChanged = table
				.getSelectedRow() != table.currentSelection[0];
	...
	protected boolean tryInsertRow(SQLiteTable table,
			Exception[] errorReference) {
		try {
			((DefaultTableModel) table.getModel())
					.addRow(new String[table.getColumnCount() + 1]);
	...
	protected boolean tryDeleteRow(SQLiteTable table,
			Exception[] errorReference) {
		try {
			TreeMap<String, Object> primKeyVal = new TreeMap<String, Object>();
			primKeyVal.put("_rowid_",
					(String) table.getValueAt(table.currentSelection[0],
	...
	protected void switchToErrorUpdatingState(SQLiteTable table,
		...
		JOptionPane.showMessageDialog(frmSqliteDataEditor, error.getMessage(),
				"Error", JOptionPane.ERROR_MESSAGE);
	}
	...
	protected Color[] getCurrentSelectionColors() {
		switch (state) {
			case DEFAULT_STATE:
				return new Color[] { DEFAULT_TABLE_ROW_SELECTION_COLOR,
						Color.WHITE };
			case UPDATING_STATE:
