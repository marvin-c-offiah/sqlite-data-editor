					...
					if (selectedIdx > -1) {
						...
						if (selectedRows.length > 0) {
							for (int row : selectedRows) {
								...
								primKeyVal.put("_rowid_", (String) table.getValueAt(row,
										table.getColumnModel().getColumnIndex("_rowid_")));
	...
	public void update(Observable database, Object object) {
		...
		try {
			...
			for (int i = 0; i < tblNames.length; i++) {
				...
				String[] colNames = ksColNames.toArray(new String[0]);
				...
				String[] refTblNames = importedKeys.keySet().toArray(new String[0]);
				...
				tbl.removeColumn(tbl.getColumn("_rowid_"));
							...
							tcm.getColumn(
									...
									.setCellEditor(
											new DefaultCellEditor(new JComboBox(((SQLiteInMemoryDatabase) database)
													.selectColumns(refTblName, new String[] { colName }, true)
													.get(colName))));
						} else {
						...
						try {
							...
							if (selectedRow != lastSelectedTableRow) {
								if (isInserting) {
									...
									int rowId = (int) tbl.getModel().getValueAt(lastSelectedTableRow, 0);
